<template>
  <div class="question">
    ...
    <div class="options">
      <div v-if="question.setting">
        <input
          class="option"
          placeholder="请输入选项"
          v-for="(opt, index) in question.setting.options"
          :key="index+createId()"
          v-model="opt.title"
        />
        <div class="add-option" @click="addOption"><img src="./assets/addOption.png">添加选项</div>
      </div>
    </div>
...
export default {
  name: "selectQuestion",
  props: ["q_index", "problem","delete","createId","problemTypes"],
  data() {
    return {
  ...
  methods: {
    ...
    addOption(){
      this.question.setting.options.push({title: "默认选项", status: 2})
    }
  },
};
</script>
